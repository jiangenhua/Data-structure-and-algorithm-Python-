...
class ExpressionTree(LinkedBinaryTree):
  ...
  def _evaluate_recur(self, p):
    ...
    if self.is_leaf(p):
      ...
      if op == '+':
        print('left_val + right_val:',left_val + right_val)
...
def tokenize(raw):             # 将一连串表达式拆分为单个字符串格式
...
def build_expression_tree(tokens):              # 接受单个字符串格式的表达式，并将其构建为树
...
if __name__ == '__main__':
  big = build_expression_tree(tokenize('((2x(9-1))+(3x10))'))
